...
function calcTwoPairs(diceCount){
  let tempCount = diceCount;
  let pairsFound = 0, index = 6, sum = 0;
  while (pairsFound < 2 && index > 0) {
    let score = calcIdentical(tempCount, 2);
    tempCount[index] = 0;
    sum += score;
    if (score !== 0) {
      pairsFound++;
...
function calcStraight(diceCount,start,stop){
  let filteredArray = diceCount.filter((value, index) => {
    return (index >= start && index <= stop)
  });
  let IndexsWithDiceValues = filteredArray.reduce((sum, val) => sum + (val > 0 ? 1 : 0))
  if (IndexsWithDiceValues === stop - start + 1) {
    let sum = 0;
...
function calcChance(diceCount){
  let sum = 0, diceCounted = 0, index = 6;
  while (diceCounted < 5 && index > 0) {
    let diceToCount = Math.min(diceCount[index], (5 - diceCounted));
    sum += diceToCount * index;
    diceCounted += diceToCount;
    index--;}
  return sum;
}
