...
import {yatzyHomePage,newYatzyGamePage,newYatzyRoundPage,validateYatzyConfigData,
  validateYatzyRoundData,validateYatzyHighScoreData, yatzyHighScorePage, ValidationError} from "./yatzy-site-forms-handling.js";
...
/* ToDO:protect againts DOS attack from malicious user sending an very very large post body.
if (body.length > 1e6) { 
...
function processReq(req,res){
  ...
  let sp=new URLSearchParams(u.search);
  let queryPath=decodeURIComponent(u.pathname); //Convert uri encoded special letters (eg æøå that is escaped by "%number") to JS string
  ...
  switch(req.method){
    ...
    switch(pathElements[1]){  //index 0 contains string before first "/" (which is empty) 
      ...
      break;
        case "highscores":
          try {
           let validatedData= validateYatzyHighScoreData(sp);
        ...
        default: //for anything else we assume it is a file to be served
          let response = fileResponse(res, queryPath);
          return response;
        break;
       }
     }
